...
space = {
    ...
    # 'numoflayer': hp.choice('numoflayer', [1, 2]),
    'dropout': hp.uniform('dropout', 0.1, 0.5),
    'kernel_size': hp.choice('kernel_size', [3, 5, 7]),
    'numoffilters': hp.choice('numoffilters', [1, 2, 3, 4, 5]),
    'activation' : hp.choice('activation', ['relu', 'tanh', 'softplus', 'softsign']),
    'numofstrides' : hp.choice('numofstrides', [1, 2, 3]),
    'stft_len' : hp.choice('stft_len', [16, 32, 64, 128]),
...
def create_model(x_train, y_train, x_test, y_test, space, input_shape):
    ...
    with open('output.csv', 'a', newline='') as csvfile:
        ...
        writer.writerow([numoffilter, kernal_size, numofstrides, batch_size, activation, numoflayer, numofnode4, dropout, lr, epoch_times, stft_len, overlap, acc])
